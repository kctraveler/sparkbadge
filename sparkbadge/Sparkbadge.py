...
class Sparkbadge:
    def __init__(self, 
        ...
        self._sparkline = trend(samples=metric_data, stroke_color=line_color, stroke_width=line_width)
        with tempfile.NamedTemporaryFile(mode="w", suffix=".svg") as line:
            line.write(self._sparkline)
            line.flush()
            self.badge = badge(center_image=line.name,
